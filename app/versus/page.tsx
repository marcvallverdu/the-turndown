...
export default function VersusPage() {
  const articles = getArticlesByCategory('versus');

  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 pb-20 pt-12">
      <section>
        <p className="text-xs uppercase tracking-[0.3em] text-charcoal/50">Versus</p>
        <h1 className="mt-4 font-serif text-5xl">Head-to-head on the best addresses.</h1>
      </section>
      <section className="grid gap-6 md:grid-cols-2">
        {articles.map((article) => (
          <Link
            key={article.slug}
            href={`/versus/${article.slug}`}
          ...
          >
            <div className="relative h-52">
              ...
              <div className="absolute bottom-4 left-4 text-white">
                <p className="text-xs uppercase tracking-[0.3em] text-white/70">Versus</p>
