...
# Usage: scripts/seed-production.sh scripts/seed-may30.ts capri
#
...
if [[ -z "$SEED_FILE" || -z "$EXPECTED_SLUG" ]]; then
  echo "Usage: $0 <seed-file> <expected-slug>" >&2
  exit 2
fi
...
ssh -o BatchMode=yes -o ConnectTimeout=10 "$PROD_HOST" bash -s -- "$SEED_FILE" "$EXPECTED_SLUG" "$APP_PREFIX" "$DB_CONTAINER" "$DB_USER" "$DB_NAME" <<'REMOTE'
set -euo pipefail
seed_file="$1"
expected_slug="$2"
app_prefix="$3"
...
row="$(docker exec "$db_container" psql -U "$db_user" -d "$db_name" -tAc "select slug || chr(9) || name || chr(9) || published from destinations where slug = '$expected_slug';")"
if [[ -z "$row" ]]; then
  echo "Verification failed: slug $expected_slug not found in production database." >&2
  exit 1
fi
printf '%s\n' "$row"
REMOTE
